...
beta_list = [0.03+0.02*i for i in range(10)]
...
for kk in num_list:
    ...
    plt.plot(beta_list,maglist,'+', label = 'simulation')

    ideal_lsit = [cal_ma_ideal(i) for i in xx_list]
    plt.plot(xx_list, yy_list,'-', label = 'theory')
    plt.legend()
    #plt.plot(beta_list, cv_list, '*')
    plt.title('magnetism')
